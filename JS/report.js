document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".select-list button, .chips .pill").forEach((item) => {
    item.addEventListener("click", () => ABC.activateInGroup(item, item.tagName === "BUTTON" ? "button" : ".pill"));
  });
  ...
  document.querySelector(".report-form .primary")?.addEventListener("click", (event) => {
    ...
    window.setTimeout(() => {
      const type = document.querySelector(".select-list .active")?.textContent.trim() || "현황 분석";
      const period = document.querySelector(".chips .active")?.textContent.trim() || "최근 3년";
      document.querySelector(".report-page header h2").textContent = `도로 파손 ${type.replace(/[▥▤▢]/g, "").trim()} 보고서`;
      document.querySelector(".report-page header span").textContent = `생성일 2026.6.22 · 소스 3개 · ${period}`;
      done();
